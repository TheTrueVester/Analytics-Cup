getwd()
setwd("D:/CourseResources/BA&ML/AnalysisCup/training_data")
...
# split labeled data into training set and validation set
train_set_split = initial_split(df,prop = 0.8,strata = Reseller)
train_set = training(train_set_split)
val_set = testing(train_set_split)
#data_balanced_over <- ovun.sample(Reseller ~ ., data = train_set, method = 'over',N = 500)$data
#table(data_balanced_over$cls)
train_set <- train_set %>% select(Customer_ID, order_count, avg_item, sum_item, sum_kinds, avg_net_value, Order,
                                  Contract, Credit_memo, Returns, Reseller)
summary(train_set)
summary(val_set)
train_set <- ovun.sample(Reseller ~ ., data = train_set, method = "over",N = 1960)$data
table(train_set$Reseller)
?ovun.sample
################################################################################
#############################   Model Selection  ###############################
################################################################################
...
PN <- count(train_set_with_predictions %>% filter(.pred_class == "N"))
PP <- count(train_set_with_predictions %>% filter(.pred_class == "Y"))
RN <- count(train_set_with_predictions %>% filter(Reseller == "N"))
RP <- count(train_set_with_predictions %>% filter(Reseller == "Y"))

TP <- count(train_set_with_predictions %>% filter(.pred_class == "Y" & Reseller == "Y"))
TP
TN <- count(train_set_with_predictions %>% filter(.pred_class == "N" & Reseller == "N"))
TN
FP <- count(train_set_with_predictions %>% filter(.pred_class == "Y" & Reseller == "N"))
FP
FN <- count(train_set_with_predictions %>% filter(.pred_class == "N" & Reseller == "Y"))
FN
...
PN <- count(val_preds %>% filter(.pred_class == "N"))
PP <- count(val_preds %>% filter(.pred_class == "Y"))
RN <- count(val_preds %>% filter(Reseller == "N"))
RP <- count(val_preds %>% filter(Reseller == "Y"))

TP <- count(val_preds %>% filter(.pred_class == "Y" & Reseller == "Y"))
TP
TN <- count(val_preds %>% filter(.pred_class == "N" & Reseller == "N"))
TN
FP <- count(val_preds %>% filter(.pred_class == "Y" & Reseller == "N"))
FP
FN <- count(val_preds %>% filter(.pred_class == "N" & Reseller == "Y"))
FN
sensitivity <- TP / (TP + FN)
sensitivity
specificity <- TN / (FP + TN)
...
write_csv(submission, "predictions_Hanuta.csv")
